...
public class BvhTriangleMeshShape : TriangleMeshShape
{
    ...
    public BvhTriangleMeshShape(StridingMeshInterface meshInterface, bool useQuantizedAabbCompression, bool buildBvh = true)
    {
        IntPtr native = btBvhTriangleMeshShape_new(meshInterface.Native, useQuantizedAabbCompression, buildBvh);
    ...
    {
        IntPtr native = btBvhTriangleMeshShape_new2(meshInterface.Native, useQuantizedAabbCompression, ref bvhAabbMin, ref bvhAabbMax, buildBvh);
        InitializeCollisionShape(native);
        InitializeMembers(meshInterface);
    }
    ...
    public void PerformConvexcast(TriangleCallback callback, Vector3 boxSource, Vector3 boxTarget, Vector3 boxMin, Vector3 boxMax)
        => btBvhTriangleMeshShape_performConvexcast(Native, callback.Native, ref boxSource, ref boxTarget, ref boxMin, ref boxMax);

    public void PerformRaycast(TriangleCallback callback, Vector3 raySource, Vector3 rayTarget)
        => btBvhTriangleMeshShape_performRaycast(Native, callback.Native, ref raySource, ref rayTarget);
