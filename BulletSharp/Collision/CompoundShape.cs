...
internal struct CompoundShapeData
{
    ...
    public static int Offset(string fieldName) => Marshal.OffsetOf(typeof(CompoundShapeData), fieldName).ToInt32();
}
...
internal struct CompoundShapeChildData
{
    ...
    public static int Offset(string fieldName) => Marshal.OffsetOf(typeof(CompoundShapeChildData), fieldName).ToInt32();
}
...
public class CompoundShape : CollisionShape
{
    ...
    public void CalculatePrincipalAxisTransform(float[] masses, ref Matrix4x4 principal, out Vector3 inertia)
        => btCompoundShape_calculatePrincipalAxisTransform(Native, masses, ref principal, out inertia);

    public void CreateAabbTreeFromChildren()
        => btCompoundShape_createAabbTreeFromChildren(Native);

    public CollisionShape GetChildShape(int index)
        => ChildList[index].ChildShape;
    ...
    public void UpdateChildTransform(int childIndex, Matrix4x4 newChildTransform, bool shouldRecalculateLocalAabb = true)
        => btCompoundShape_updateChildTransform(Native, childIndex, ref newChildTransform, shouldRecalculateLocalAabb);
}
