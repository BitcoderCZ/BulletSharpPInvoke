...
public class GImpactPair : BulletDisposableObject
{
    ...
    protected override void Dispose(bool disposing) => GIM_PAIR_delete(Native);
}
...
public class BvhTree : BulletDisposableObject
{
    ...
    public GimBvhTreeNode GetNodePointer()
        => new GimBvhTreeNode(btBvhTree_get_node_pointer(Native), this);

    public GimBvhTreeNode GetNodePointer(int index)
        => new GimBvhTreeNode(btBvhTree_get_node_pointer2(Native, index), this);

    public int GetEscapeNodeIndex(int nodeIndex)
        => btBvhTree_getEscapeNodeIndex(Native, nodeIndex);

    public int GetLeftNode(int nodeIndex)
        => btBvhTree_getLeftNode(Native, nodeIndex);

    public void GetNodeBound(int nodeIndex, Aabb bound)
        => btBvhTree_getNodeBound(Native, nodeIndex, bound.Native);

    public int GetNodeData(int nodeIndex)
        => btBvhTree_getNodeData(Native, nodeIndex);

    public int GetRightNode(int nodeIndex)
        => btBvhTree_getRightNode(Native, nodeIndex);

    public bool IsLeafNode(int nodeIndex)
        => btBvhTree_isLeafNode(Native, nodeIndex);
...
public class PrimitiveManagerBase : BulletDisposableObject
{
    ...
    public void GetPrimitiveBox(int primitiveIndex, Aabb primitiveBox) => btPrimitiveManagerBase_get_primitive_box(Native, primitiveIndex, primitiveBox.Native);

    public void GetPrimitiveTriangle(int primitiveIndex, PrimitiveTriangle triangle) => btPrimitiveManagerBase_get_primitive_triangle(Native, primitiveIndex, triangle.Native);

    protected override void Dispose(bool disposing) => btPrimitiveManagerBase_delete(Native);
}
...
public class GImpactBvh : BulletDisposableObject
{
    ...
    public static void FindCollision(GImpactBvh boxSet1, ref Matrix4x4 transform1, GImpactBvh boxSet2, ref Matrix4x4 transform2, PairSet collisionPairs)
        => btGImpactBvh_find_collision(boxSet1.Native, ref transform1, boxSet2.Native, ref transform2, collisionPairs.Native);

    public void BuildSet()
        => btGImpactBvh_buildSet(Native);

    public GimBvhTreeNode GetNodePointer(int index = 0)
        => new GimBvhTreeNode(btGImpactBvh_get_node_pointer(Native, index), this);

    public int GetEscapeNodeIndex(int nodeIndex)
        => btGImpactBvh_getEscapeNodeIndex(Native, nodeIndex);

    public int GetLeftNode(int nodeIndex)
        => btGImpactBvh_getLeftNode(Native, nodeIndex);

    public void GetNodeBound(int nodeIndex, Aabb bound)
        => btGImpactBvh_getNodeBound(Native, nodeIndex, bound.Native);

    public int GetNodeData(int nodeIndex)
        => btGImpactBvh_getNodeData(Native, nodeIndex);
    ...
    public int GetRightNode(int nodeIndex)
        => btGImpactBvh_getRightNode(Native, nodeIndex);

    public bool IsLeafNode(int nodeIndex)
        => btGImpactBvh_isLeafNode(Native, nodeIndex);
