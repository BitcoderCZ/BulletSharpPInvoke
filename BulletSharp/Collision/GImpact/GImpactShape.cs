...
public abstract class GImpactShapeInterface : ConcaveShape
{
    ...
    public void GetChildAabb(int childIndex, Matrix4x4 transform, out Vector3 aabbMin, out Vector3 aabbMax)
        => btGImpactShapeInterface_getChildAabb(Native, childIndex, ref transform, out aabbMin, out aabbMax);
    ...
    public void ProcessAllTrianglesRayRef(TriangleCallback callback, ref Vector3 rayFrom, ref Vector3 rayTo)
        => btGImpactShapeInterface_processAllTrianglesRay(Native, callback.Native, ref rayFrom, ref rayTo);

    public void ProcessAllTrianglesRay(TriangleCallback callback, Vector3 rayFrom, Vector3 rayTo)
        => btGImpactShapeInterface_processAllTrianglesRay(Native, callback.Native, ref rayFrom, ref rayTo);
...
{
    private readonly List<CollisionShape> _childShapes = [];
...
public sealed class TrimeshPrimitiveManager : PrimitiveManagerBase
{
    ...
    {
        IntPtr native = btGImpactMeshShapePart_TrimeshPrimitiveManager_new(meshInterface.Native, part);
    ...
    public void GetIndices(int faceIndex, out uint i0, out uint i1, out uint i2b)
        => btGImpactMeshShapePart_TrimeshPrimitiveManager_get_indices(Native, faceIndex, out i0, out i1, out i2b);

    public void GetVertex(uint vertexIndex, out Vector3 vertex)
        => btGImpactMeshShapePart_TrimeshPrimitiveManager_get_vertex(Native, vertexIndex, out vertex);
...
public class GImpactMeshShape : GImpactShapeInterface
{
    ...
    public GImpactMeshShapePart GetMeshPart(int index)
        => new GImpactMeshShapePart(btGImpactMeshShape_getMeshPart(Native, index), this);
}
...
#pragma warning restore SA1201 // Elements should appear in the correct order
{
    ...
    public static int Offset(string fieldName)
        => Marshal.OffsetOf(typeof(GImpactMeshShapeData), fieldName).ToInt32();
}
