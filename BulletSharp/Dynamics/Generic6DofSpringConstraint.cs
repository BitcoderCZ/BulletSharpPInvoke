...
internal unsafe struct Generic6DofSpringConstraintFloatData
{
    ...
    public static int Offset(string fieldName)
        => Marshal.OffsetOf(typeof(Generic6DofSpringConstraintFloatData), fieldName).ToInt32();
}
...
internal unsafe struct Generic6DofSpringConstraintDoubleData
{
    ...
    public static int Offset(string fieldName)
        => Marshal.OffsetOf(typeof(Generic6DofSpringConstraintDoubleData), fieldName).ToInt32();
}

public class Generic6DofSpringConstraint : Generic6DofConstraint
{
    public Generic6DofSpringConstraint(RigidBody rigidBodyA, RigidBody rigidBodyB, Matrix4x4 frameInA, Matrix4x4 frameInB, bool useLinearReferenceFrameA)
    {
        IntPtr native = btGeneric6DofSpringConstraint_new(rigidBodyA.Native, rigidBodyB.Native, ref frameInA, ref frameInB, useLinearReferenceFrameA);
        InitializeUserOwned(native);
        InitializeMembers(rigidBodyA, rigidBodyB);
    }

    public Generic6DofSpringConstraint(RigidBody rigidBodyB, Matrix4x4 frameInB, bool useLinearReferenceFrameB)
    {
        IntPtr native = btGeneric6DofSpringConstraint_new2(rigidBodyB.Native, ref frameInB, useLinearReferenceFrameB);
        InitializeUserOwned(native);
    ...
    public float GetDamping(int index)
        => btGeneric6DofSpringConstraint_getDamping(Native, index);

    public float GetEquilibriumPoint(int index)
        => btGeneric6DofSpringConstraint_getEquilibriumPoint(Native, index);

    public float GetStiffness(int index)
        => btGeneric6DofSpringConstraint_getStiffness(Native, index);

    public bool IsSpringEnabled(int index)
        => btGeneric6DofSpringConstraint_isSpringEnabled(Native, index);
