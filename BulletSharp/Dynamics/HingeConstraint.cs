...
public class HingeConstraint : TypedConstraint
{
    ...
    public void EnableAngularMotor(bool enableMotor, float targetVelocity, float maxMotorImpulse) => btHingeConstraint_enableAngularMotor(Native, enableMotor, targetVelocity,
            maxMotorImpulse);

    public float GetHingeAngleRef(ref Matrix4x4 transA, ref Matrix4x4 transB) => btHingeConstraint_getHingeAngle(Native, ref transA, ref transB);

    public float GetHingeAngle(Matrix4x4 transA, Matrix4x4 transB) => btHingeConstraint_getHingeAngle(Native, ref transA, ref transB);

    public void GetInfo1NonVirtual(ConstraintInfo1 info) => btHingeConstraint_getInfo1NonVirtual(Native, info.Native);

    public void GetInfo2Internal(ConstraintInfo2 info, Matrix4x4 transA, Matrix4x4 transB,
        Vector3 angVelA, Vector3 angVelB) => btHingeConstraint_getInfo2Internal(Native, info.Native, ref transA,
            ref transB, ref angVelA, ref angVelB);

    public void GetInfo2InternalUsingFrameOffset(ConstraintInfo2 info, Matrix4x4 transA,
        Matrix4x4 transB, Vector3 angVelA, Vector3 angVelB) => btHingeConstraint_getInfo2InternalUsingFrameOffset(Native, info.Native,
    ...
    public void GetInfo2NonVirtual(ConstraintInfo2 info, Matrix4x4 transA, Matrix4x4 transB,
        Vector3 angVelA, Vector3 angVelB) => btHingeConstraint_getInfo2NonVirtual(Native, info.Native, ref transA,
            ref transB, ref angVelA, ref angVelB);

    public void SetAxisRef(ref Vector3 axisInA) => btHingeConstraint_setAxis(Native, ref axisInA);

    public void SetAxis(Vector3 axisInA) => btHingeConstraint_setAxis(Native, ref axisInA);

    public void SetFramesRef(ref Matrix4x4 frameA, ref Matrix4x4 frameB) => btHingeConstraint_setFrames(Native, ref frameA, ref frameB);

    public void SetFrames(Matrix4x4 frameA, Matrix4x4 frameB) => btHingeConstraint_setFrames(Native, ref frameA, ref frameB);

    public void SetLimit(float low, float high) => btHingeConstraint_setLimit(Native, low, high);

    public void SetLimit(float low, float high, float softness) => btHingeConstraint_setLimit2(Native, low, high, softness);

    public void SetLimit(float low, float high, float softness, float biasFactor) => btHingeConstraint_setLimit3(Native, low, high, softness, biasFactor);

    public void SetLimit(float low, float high, float softness, float biasFactor,
        float relaxationFactor) => btHingeConstraint_setLimit4(Native, low, high, softness, biasFactor,
            relaxationFactor);

    public void SetMotorTarget(float targetAngle, float deltaTime) => btHingeConstraint_setMotorTarget(Native, targetAngle, deltaTime);

    public void SetMotorTargetRef(ref Quaternion qAinB, float deltaTime) => btHingeConstraint_setMotorTarget2(Native, ref qAinB, deltaTime);

    public void SetMotorTarget(Quaternion qAinB, float deltaTime) => btHingeConstraint_setMotorTarget2(Native, ref qAinB, deltaTime);

    public void TestLimitRef(ref Matrix4x4 transA, ref Matrix4x4 transB) => btHingeConstraint_testLimit(Native, ref transA, ref transB);

    public void TestLimit(Matrix4x4 transA, Matrix4x4 transB) => btHingeConstraint_testLimit(Native, ref transA, ref transB);

    public void UpdateRhs(float timeStep) => btHingeConstraint_updateRHS(Native, timeStep);
...
internal struct HingeConstraintFloatData
{
    ...
    public static int Offset(string fieldName) => Marshal.OffsetOf(typeof(HingeConstraintFloatData), fieldName).ToInt32();
}
...
internal struct HingeConstraintDoubleData
{
    ...
    public static int Offset(string fieldName) => Marshal.OffsetOf(typeof(HingeConstraintDoubleData), fieldName).ToInt32();
}
